...
import { Save, Globe, MessageCircle, Settings2, Wifi, KeyRound, Eye, EyeOff, Pencil, CheckCircle2, XCircle, ImagePlus, WalletCards, Mail } from "lucide-react";
...
const DEFAULTS: SettingsForm = {
  ...
  support_email: "", footer_text: "© PCYBER ICT SERVICES. All rights reserved.",
...
export default function AdminSettings() {
  ...
  const [form, setForm] = useState<SettingsForm>(DEFAULTS), [saving, setSaving] = useState(false), [tab, setTab] = useState<"general" | "payment" | "smtp" | "logos" | "funding">("general");
  ...
  const tabs = [{ id: "general", label: "General", icon: Settings2 }, { id: "payment", label: "Payment", icon: Globe }, { id: "smtp", label: "SMTP", icon: Mail }, { id: "logos", label: "Site logo", icon: ImagePlus }, { id: "funding", label: "Manual funding", icon: WalletCards }] as const;
  return <div><div className="mb-6 flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-white">Site Settings</h1><p className="mt-0.5 text-sm text-white/40">Configure your {form.site_name.trim() || "[Site Name]"} platform.</p></div>{tab === "logos" || tab === "general" ? <Button variant="glossy" size="sm" disabled={saving} onClick={() => void handleSave()}><Save size={14} /> {saving ? "Saving…" : "Save Changes"}</Button> : null}</div>
