...
export default function AdminAnalytics() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-white/10 bg-[#1a0b30]/80 p-4 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="font-bold text-white">Revenue trend</h2><p className="mt-1 text-xs text-white/40">Completed purchases by day</p></div><span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs text-emerald-300">{money(s.purchaseRevenue)} total</span></div><div className="h-[280px] w-full">{analytics.daily.length ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={analytics.daily}><defs><linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#df20ba" stopOpacity={0.4} /><stop offset="100%" stopColor="#7519e9" stopOpacity={0} /></linearGradient></defs><CartesianGrid stroke="#ffffff12" vertical={false} /><XAxis dataKey="date" tickFormatter={(date) => date.slice(5)} tick={{ fill: "#ffffff66", fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis tickFormatter={(value) => `₦${Number(value).toLocaleString()}`} tick={{ fill: "#ffffff66", fontSize: 11 }} axisLine={false} tickLine={false} width={72} /><Tooltip contentStyle={{ background: "#24113f", border: "1px solid #ffffff22", borderRadius: 12, color: "#fff" }} formatter={(value: number) => [money(value), "Revenue"]} /><Area type="monotone" dataKey="revenue" stroke="#df20ba" strokeWidth={3} fill="url(#revenueFill)" /></AreaChart></ResponsiveContainer> : <div className="flex h-full items-center justify-center text-sm text-white/35">No completed purchases in this date range.</div>}</div></section>
